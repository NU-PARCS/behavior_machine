...
def test_default_state_name():

    s = State()
    assert s.get_name().startswith("State")

def test_default_state_name_nested():

    class UpperState(State):
        def execute(self, board):
            return StateStatus.SUCCESS
    
    s = UpperState()
    assert s.get_name().startswith("UpperState")
